...
Boid::Boid(float x, float y, int predCheck)
{
    predator = predCheck;
    if (predCheck == 1) {
        predator = 1;
...
Pvector Boid::Separation(vector<Boid> boids)
{
    ...
    for (int i = 0; i < boids.size(); i++) {
        ...
        if(predator == 0 && boids[i].predator == 1 && (d < killsep) && (d > 0)){
                killed = 1;
                predator = 2;
                maxSpeed = 0;
                maxForce = 0;
        ...
        if ((d > 0) && (d < preysep/1.5) && predator == 1
            && boids[i].predator == 1) {
        ...
        else if ((d > 0) && (d < predsep) && boids[i].predator == 1) {
            Pvector pred(0, 0);
        ...
        else if ((d > 0) && (d < obsep) && boids[i].predator == 2) {
            Pvector pred(0, 0);
            pred = pred.subTwoVector(location, boids[i].location);
            pred.mulScalar(200);
            steer.addVector(pred);
...
Pvector Boid::Alignment(vector<Boid> Boids)
{
    ...
    if(predator != 2){
        for (int i = 0; i < Boids.size(); i++) {
        float d = location.distance(Boids[i].location);
        if ((d > 0) && (d < preyatt)) { //!<  0 < d < 50
            if(Boids[i].predator == 0){
...
Pvector Boid::Cohesion(vector<Boid> Boids)
{
    ...
    if(predator != 2){
       for (int i = 0; i < Boids.size(); i++) {
        float d = location.distance(Boids[i].location);
        if ((d > 0) && (d < preyatt) && Boids[i].predator == 0 && predator == 0) {
            sum.addVector(Boids[i].location);
            count++;
        }
        if ((d > 0) && (d < predatt) && Boids[i].predator == 0 && predator == 1) {
            sum.addVector(Boids[i].location);
            count++;
        }
    }
